import 'cypress-iframe'
require ('@4tw/cypress-drag-drop')

describe('Mouse Events', () => {
    it('Mouse Hover action ', () => {
        cy.visit('https://demo.opencart.com/')
        cy.get('body > main:nth-child(3) > div:nth-child(1) > nav:nth-child(1) > div:nth-child(3) > ul:nth-child(1) > li:nth-child(1) > div:nth-child(2) > div:nth-child(1) > ul:nth-child(1) > li:nth-child(2) > a:nth-child(1)').should('not.be.visible')
        cy.get('.nav > :nth-child(1) > .dropdown-toggle').trigger('mouseover').click()
        cy.get('body > main:nth-child(3) > div:nth-child(1) > nav:nth-child(1) > div:nth-child(3) > ul:nth-child(1) > li:nth-child(1) > div:nth-child(2) > div:nth-child(1) > ul:nth-child(1) > li:nth-child(2) > a:nth-child(1)').should('be.visible')
   })
   it('Right click Mouse with trigger', () => {
    cy.visit('https://swisnl.github.io/jQuery-contextMenu/demo.html')
    cy.xpath('//span[@class="context-menu-one btn btn-neutral"]').trigger('contextmenu')
    cy.get('.context-menu-icon-copy > span').should('be.visible')
   })
   it('Right click Mouse as normal', () => {
    cy.visit('https://swisnl.github.io/jQuery-contextMenu/demo.html')
    cy.xpath('//span[@class="context-menu-one btn btn-neutral"]').rightclick()
    cy.get('.context-menu-icon-copy > span').should('be.visible')
   })  
   it('Double click with trigger event', () => {
    cy.visit('https://www.w3schools.com/tags/tryit.asp?filename=tryhtml5_ev_ondblclick3')
    cy.frameLoaded('#iframeResult')
    cy.wait(3000)
    cy.iframe('#iframeResult').find('button[ondblclick="myFunction()"]').dblclick()
    cy.iframe('#iframeResult').find('#field2').should('have.value','Hello World!')
   })  
   it('Drag and Drop', () => {
    cy.visit('http://dhtmlgoodies.com/scripts/drag-drop-custom/demo-drag-drop-3.html')
    cy.wait(3000)
    cy.get('#box6').drag('#box106',{force:true})
   }) 
   it.only('scrolling a page', () => {
    cy.visit('https://www.countries-ofthe-world.com/flags-of-the-world.html')
    cy.xpath('//td[normalize-space()="Luxembourg"]').scrollIntoView({duration:5000})
    cy.xpath('//td[normalize-space()="Luxembourg"]').should('be.visible')
    cy.wait(5000)
    cy.xpath('//td[normalize-space()="Bolivia"]').scrollIntoView({duration:5000})
    cy.xpath('//td[normalize-space()="Bolivia"]').should('be.visible',{force:true})
   }) 
})